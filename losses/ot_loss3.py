    ...
    def forward(self, normed_density, unnormed_density, points):
        ...
        for idx, im_points in enumerate(points):
            if len(im_points) > 0:
                ...
                print(dis)
                P, log = solve_semi_dual_entropic(source_prob,target_prob,dis,
                self.reg,'asgd',self.num_of_iter_in_ot,None,log=True)
                
                
                beta = log['alpha'] # size is the same as source_prob: [#cood * #cood]
