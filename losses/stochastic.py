...
def coordinate_grad_semi_dual(b, M, reg, beta, i):
    ...
    >>> X_source = cp.random.randn(n_source, 2)
    >>> Y_target = cp.random.randn(n_target, 2)
...
def sag_entropic_transport(a, b, M, reg, numItermax=10000, lr=None):
    ...
    stored_gradient = cp.zeros((n_source, n_target))
    sum_stored_gradient = cp.zeros(n_target)
    for _ in range(numItermax):
        i = cp.random.randint(n_source)
...
def batch_grad_dual(a, b, M, reg, alpha, beta, batch_size, batch_alpha,
    ...
    grad_beta[batch_beta] = (b[batch_beta] * len(batch_alpha) / cp.shape(M)[0]
                             + G.sum(0))
    grad_alpha[batch_alpha] = (a[batch_alpha] * len(batch_beta)
                               / cp.shape(M)[1] + G.sum(1))
...
def sgd_entropic_regularization(a, b, M, reg, batch_size, numItermax, lr):
    ...
    n_source = cp.shape(M)[0]
    n_target = cp.shape(M)[1]
    cur_alpha = cp.zeros(n_source)
    cur_beta = cp.zeros(n_target)
    for cur_iter in range(numItermax):
        k = cp.sqrt(cur_iter + 1)
        batch_alpha = cp.random.choice(n_source, batch_size, replace=False)
        batch_beta = cp.random.choice(n_target, batch_size, replace=False)
